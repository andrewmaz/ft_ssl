...
/*   Updated: 2018/05/21 17:27:06 by amazurok         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_r(t_md5 *h, unsigned int *wsi, \
				unsigned int (*f1)(unsigned int, unsigned int, unsigned int))
{
	...
	tmp = h->v + ((*f1)(h->next->v, h->next->next->v, h->next->next->next->v) \
			+ wsi[0] + k[wsi[2]]);
	tmp = (((tmp) << (wsi[1])) | ((tmp) >> (32 - (wsi[1]))));
	h->v = h->next->v + tmp;
	free(wsi);
}
